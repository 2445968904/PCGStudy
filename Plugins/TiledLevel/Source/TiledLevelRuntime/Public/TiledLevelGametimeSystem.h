...
/*
 * TODO: Add gameplay logic on particular tile positions?
 * ex: GetTilePosition from OOOXXX
 * BuildItem at position, Remove item at position
...
public:
	...
	bool InitGametimeSystem_Internal(const UObject* WorldContextObject, UTiledItemSet* StartupItemSet);


	// TODO: set the included levels in each level actor is more intuitive ?
	...
	*/
	...
	// ... No need for this, there is GametimeData for this purpose, for editor time? they may just use all the core features!
...
private:
	...
	UPROPERTY()
	TObjectPtr<UTiledItemSet> SourceItemSet;
	
	UPROPERTY()
	TObjectPtr<ATiledLevel> GametimeLevel;

	UPROPERTY()
	TObjectPtr<class ATiledLevelEditorHelper> Helper;
