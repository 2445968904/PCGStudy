	...
	: Super(ObjectInitializer)
{
	...
	ProcedurePreviewGrids = CreateDefaultSubobject<UProceduralMeshComponent>(TEXT("FillPreviewGrids"));
	ProcedurePreviewGrids->CastShadow = false;
	ProcedurePreviewGrids->SetupAttachment(Root);
	...
	ConstructorHelpers::FObjectFinder<UStaticMesh> Asset_Cube(TEXT("/Engine/BasicShapes/Cube"));
	SM_Cube = Asset_Cube.Object;
...
void ATiledLevelEditorHelper::MirrorPaintBrush(EAxis::Type MirrorAxis,  bool& MirrorXState, bool& MirrorYState, bool& MirrorZState)
{
    ...
    switch (ActiveItem->PivotPosition)
    {
        case EPivotPosition::Bottom:
            ...
            {
                if (MirrorZState)
                    Center->AddLocalOffset(TileSize * ActiveItem->Extent * FVector(0, 0, BottomMod));
                else
                    Center->AddLocalOffset(TileSize * ActiveItem->Extent * -FVector(0, 0, BottomMod));
            }
            break;
        ...
        case  EPivotPosition::Corner:
            {
                ...
                {
                    if (MirrorXState)
                        Center->AddLocalOffset(TileSize * ActiveItem->Extent * XMod * FVector(1, 0, 0));
                    else
                        Center->AddLocalOffset(TileSize * ActiveItem->Extent * XMod * -FVector(1, 0, 0));
                }
                ...
                {
                    if (MirrorYState)
                        Center->AddLocalOffset(TileSize * ActiveItem->Extent * YMod * FVector(0, 1, 0));
                    else
                        Center->AddLocalOffset(TileSize * ActiveItem->Extent * YMod * -FVector(0, 1, 0));
                }
                if (MirrorAxis == EAxis::Z)
        ...
        case EPivotPosition::Side:
            if (MirrorAxis == EAxis::X)
            {
                if (MirrorXState) // rotate? H vs V
                    Center->AddLocalOffset(TileSize * ActiveItem->Extent * FVector(1, 0, 0));
                else
                    Center->AddLocalOffset(TileSize * ActiveItem->Extent * -FVector(1, 0, 0));
            }
            break;
        case EPivotPosition::Fit: 
            ...
            {
                if (MirrorZState)
                    Center->AddLocalOffset(TileSize * ActiveItem->Extent * FVector(0, 0, 1));
                else
                    Center->AddLocalOffset(TileSize * ActiveItem->Extent * -FVector(0, 0, 1));
            }
            break;
...
void ATiledLevelEditorHelper::SetupEraserBrush(FIntVector EraserExtent, EPlacedType EraserShape)
{
...
void ATiledLevelEditorHelper::ResetBrush()
{
	...
	ProcedurePreviewGrids->ClearAllMeshSections();
...
void ATiledLevelEditorHelper::RotateBrush(bool IsCW, EPlacedType PlacedType, bool& ShouldRotateExtent)
{
	...
	EPlacedShapeType ShapeType = FTiledLevelUtility::PlacedTypeToShape(PlacedType);
	...
	case 1:
		...
		Brush->SetRelativeLocation(CachedBrushLocation);
		if (ShapeType == Shape3D)
		{
			Brush->AddWorldOffset(F.TransformPosition(TileSize * TempExtent * FVector(1, 0, 0)));
		}
	...
	case 2:
		...
		if (ShapeType == Shape3D)
		{
			Brush->AddWorldOffset(F.TransformPosition(TileSize * TileExtent * FVector(1, 1, 0)));
			
		}
		else if (ShapeType == Shape2D)
		{
			Brush->AddWorldOffset(F.TransformPosition(TileSize * TileExtent * FVector(1, 0, 0)));
		}
		ShouldRotateExtent = false;
		break;
	case 3:
		...
		if (ShapeType == Shape3D)
		{
			Brush->AddWorldOffset(F.TransformPosition(TileSize * TempExtent * FVector(0, 1, 0)));
		}
		else if (ShapeType == Shape2D)
		{
			Brush->AddWorldOffset(F.TransformPosition(TileSize * TileExtent * FVector(0, 1, 0)));
		}
		ShouldRotateExtent = true;
		break;
	default: ;
...
void ATiledLevelEditorHelper::UpdateFillPreviewGrids(TArray<FIntPoint> InFillBoard,int InFillFloorPosition, int InFillHeight)
{
	...
	ProcedurePreviewGrids->CreateMeshSection(0, PreviewVertices, PreviewTriangles, PreviewNormals, PreviewUVs, TArray<FColor>{}, PreviewTangents, false);
	ProcedurePreviewGrids->SetMaterial(0, M_FillPreview);
}
...
void ATiledLevelEditorHelper::SetupAutoPaintBrush(FLinearColor ItemColor)
{
	Center->ResetRelativeTransform();
	PreviewMesh->SetRelativeLocation(FVector(0));
	Brush->SetVisibility(true);
	PreviewMesh->SetVisibility(true);
	Brush->SetBoxColor(ItemColor.ToFColor(false));
	Brush->SetBoxExtent(TileSize * 0.5);
	Brush->SetRelativeLocation(TileSize * 0.5);
	PreviewMesh->SetStaticMesh(SM_Cube);
	PreviewMesh->SetRelativeScale3D(TileSize / 100.f);
	...
	M_Hint->SetVectorParameterValue("BorderColor", ItemColor);
	M_Hint->SetScalarParameterValue("BorderWidth", 0.5f);
	PreviewMesh->SetMaterial(0, M_Hint);
}

void ATiledLevelEditorHelper::ApplyAdditionalAutoPaintTransform(const FTransform& AdditionalTransform) const
{
	PreviewMesh->AddRelativeLocation(AdditionalTransform.GetTranslation());
	Center->AddRelativeRotation(AdditionalTransform.GetRotation());
	Center->SetRelativeScale3D(Center->GetComponentScale() * AdditionalTransform.GetScale3D());
}
