...
bool UTiledLevelGametimeSystem::InitGametimeSystem_Internal(const UObject* WorldContextObject, UTiledItemSet* StartupItemSet)
{
...
void UTiledLevelGametimeSystem::InitializeGametimeSystemFromData(const UObject* WorldContextObject, UTiledItemSet* StartupItemSet,
	const FTiledLevelGameData& InData, TArray<ATiledLevel*> TiledLevelsInsideData, bool bUnbound)
{
	if (!InitGametimeSystem_Internal(WorldContextObject, StartupItemSet)) return;
	...
	FActorSpawnParameters SpawnParams;
	SpawnParams.bNoFail = true; // this stuck me so long... force it spawn... no matter the collision...
	GametimeLevel = GetWorld()->SpawnActor<ATiledLevel>(ATiledLevel::StaticClass(), FVector(0, 0, 0), FRotator(0), SpawnParams);
...
void UTiledLevelGametimeSystem::ActivatePreviewItem(UTiledLevelItem* PreviewItem)
{
	...
	{
		ERROR_LOG("Error! system is not init")
		return;
	}
	if (!PreviewItem)
	{
	...
	if (!GametimeLevel)
	{
		ERROR_LOG("Error! Gametime level not exist...")
		return;
	}
...
void UTiledLevelGametimeSystem::EraseItem()
{
	...
	if (EraserShape == EPlacedShapeType::Shape2D || EraserType == EPlacedType::Any)
	{
		 ...
		 {
		 	  bool Con = EraserType == EPlacedType::Any?
			 	  FTiledLevelUtility::IsEdgeInsideTile(Placement.Edge, FIntVector(Placement.GetItem()->Extent), CurrentTilePosition, EraserExtent) :
			      FTiledLevelUtility::IsEdgeOverlapping(CurrentEdge, FVector(EraserExtent), Placement.Edge, Placement.GetItem()->Extent);
			  if (Con)
			  {
