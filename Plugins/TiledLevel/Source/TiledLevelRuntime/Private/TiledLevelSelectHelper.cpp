...
void ATiledLevelSelectHelper::PopulateSelected(bool bForTemplate)
{
    ...
    TArray<FItemPlacement> AllSelected;
    if (bForTemplate)
    {
        DisplayStatus = Template;
        AllSelected.Append(TemplateTiles);
        AllSelected.Append(TemplateEdges);
        AllSelected.Append(TemplatePoints);
    }
    else
    {
        DisplayStatus = Selection;
        AllSelected.Append(SelectedTiles);
        AllSelected.Append(SelectedEdges);
        AllSelected.Append(SelectedPoints);
    }
    for (FItemPlacement& P : AllSelected)
    {
        ...
        {
            // TODO: Can not make good visual hint for restriction item!!! All approach just failed... ignore for now... Otherwise I'll stuck here forever...
            if (UTiledLevelRestrictionItem* RestrictionItem = Cast<UTiledLevelRestrictionItem>(Item)) continue;
...
void ATiledLevelSelectHelper::GetSelectedPlacements(TArray<FTilePlacement>& OutTiles, TArray<FEdgePlacement>& OutEdges,
...
FIntVector ATiledLevelSelectHelper::GetSelectedExtent(bool bForTemplate)
{
...
TArray<FTilePlacement> ATiledLevelSelectHelper::GetModifiedTiles(bool IsFromTemplate)
{
    ...
    for (FTilePlacement CopiedTile : CopiedTiles)
    {
        ...
        case 2:
            ...
            CopiedTile.GridPosition = FIntVector(TargetExtent.X - InitPos.X - 1 - (OriginExtent.X - 1), TargetExtent.Y - InitPos.Y - 1 - (OriginExtent.Y - 1),
                                                 InitPos.Z);
            break;
        case 3:
