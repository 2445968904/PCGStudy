 ...
 */
bool FTiledLevelUtility::GetFeasibleFillTile(UTiledLevelItem* InItem, int& RotationIndex,
	TArray<FIntPoint>& CandidatePoints, FIntPoint& OutPoint)
{
	FIntPoint CheckExtent = (RotationIndex == 1 || RotationIndex == 3)?
	...
	for (FIntPoint& P : CandidatePoints)
	{
		...
		for (int x = 0; x < CheckExtent.X; x++)
		{
			...
			{
				Pass = CandidatePoints.Contains(FIntPoint(P.X + x , P.Y + y));
				if (!Pass) break;
	...
	if (!Pass && InItem->bAllowRandomRotation)
	{
		...
		CheckExtent = (RotationIndex == 1 || RotationIndex == 3)?
										  FIntPoint(int(InItem->Extent.Y), int(InItem->Extent.X)) : FIntPoint(int(InItem->Extent.X), int(InItem->Extent.Y));
		Pass = false;
		for (FIntPoint& P : CandidatePoints)
		{
...
TArray<FTiledLevelEdge> FTiledLevelUtility::GetEdgesAroundArea(const TSet<FIntPoint>& Region, int Z, bool AskForOuter)
{
...
TArray<FTiledLevelEdge> FTiledLevelUtility::GetEdgesAroundArea(const TSet<FIntVector>& Region, bool AskForOuter)
{
	TArray<FTiledLevelEdge> OutEdges;
...
TSet<FIntVector> FTiledLevelUtility::GetPointsAroundArea(const TSet<FIntVector>& Region, bool AskForOuter)
{
...
// TODO: add allow 90x degrees rotations ... Apply rotation to local placements? V2.3
...
/*
 * Normal flip error is due to **Triangles** the normals are totally fine...
 */
void FTiledLevelUtility::GenerateCubesWithUniqueFaces(TArray<FIntVector> CubePositions, FVector CubeSize, TArray<FVector>& OutVertices,
	TArray<int32>& OutTriangles, TArray<FVector>& OutNormals, TArray<FVector2D>& OutUVs, float PaddingSize)
{
	...
	struct FCubeFace
	{
		...
		FCubeFace(const FIntVector& InP0, const FIntVector& InP1, const FIntVector& InP2, const FIntVector& InP3, const FVector& InFaceNormal )
			: P0(InP0), P1(InP1), P2(InP2), P3(InP3), FaceNormal(InFaceNormal)
		{}

		void SetUV(const FVector2D& InUV0, const FVector2D& InUV1, const FVector2D& InUV2, const FVector2D& InUV3)
		{
			UV0 = InUV0;
			UV1 = InUV1;
			UV2 = InUV2;
		...
		TArray<FVector> GetActualPositions(const FVector& InCubeSize, const float InPaddingSize) const
		{
			const FVector CubeCenter = (FVector(P0) + FVector(P1) + FVector(P2) + FVector(P3)) / 4 - 0.5 * FaceNormal;
			const FVector Scale = (InCubeSize + InPaddingSize) / InCubeSize;
			return {
				((FVector(P0) - CubeCenter) * Scale + CubeCenter) * InCubeSize, 
				((FVector(P1) - CubeCenter) * Scale + CubeCenter) * InCubeSize, 
				((FVector(P2) - CubeCenter) * Scale + CubeCenter) * InCubeSize, 
				((FVector(P3) - CubeCenter) * Scale + CubeCenter) * InCubeSize
				};
		}
	...
	const FVector FrontNormal = {-1.f, 0.f, 0.f};
	const FVector BackNormal = {1.f, 0.f, 0.f};
	const FVector RightNormal = {0.f, 1.f, 0.f};
	const FVector LeftNormal = {0.f, -1.f, 0.f};
	const FVector TopNormal = {0.f, 0.f, 1.f};
	const FVector BottomNormal = {0.f, 0.f, -1.f};
	for (FIntVector Position : CubePositions)
	{
		// The six faces
		//front
		FCubeFace FrontFace(Position, Position+Yp1,Position+Zp1, Position+YZp1, FrontNormal);
		...
		FCubeFace BackFace(Position+XYZp1, Position+XYp1,Position+XZp1, Position+Xp1, BackNormal);
		...
		//right
		FCubeFace RightFace(Position+Yp1, Position+XYp1,Position+YZp1, Position+XYZp1, RightNormal);
		...
		//left
		FCubeFace LeftFace(Position+XZp1, Position+Xp1,Position+Zp1, Position, LeftNormal);
		...
		//top
		FCubeFace TopFace(Position+XZp1, Position+Zp1,Position+XYZp1, Position+YZp1, TopNormal);
		...
		//bottom
		FCubeFace BottomFace(Position, Position+Xp1,Position+Yp1, Position+XYp1, BottomNormal);
	...
	// now, there is no duplicate faces, let's create triangles and normals...
	int FaceCount = 0;
	for (FCubeFace Face : CubeFaces)
	{
		for (FVector P : Face.GetActualPositions(CubeSize, PaddingSize))
			OutVertices.Add(P);
		OutTriangles.Add(FaceCount * 4 + 0);
		OutTriangles.Add(FaceCount * 4 + 1);
		OutTriangles.Add(FaceCount * 4 + 2);
		OutTriangles.Add(FaceCount * 4 + 2);
		OutTriangles.Add(FaceCount * 4 + 1);
		OutTriangles.Add(FaceCount * 4 + 3);
		OutNormals.Add(Face.FaceNormal);
		OutNormals.Add(Face.FaceNormal);
		OutNormals.Add(Face.FaceNormal);
		OutNormals.Add(Face.FaceNormal);
		OutUVs.Add(Face.UV0);
...
int FTiledLevelUtility::GetPMC_SectionIdFromHit(FHitResult& InHit)
{
	if (UProceduralMeshComponent* PM = Cast<UProceduralMeshComponent>(InHit.Component))
	{
		if (InHit.FaceIndex == INDEX_NONE)
		{
			ERROR_LOG("You forget to set bReturnFaceIndex to true in your FCollisionQueryParams")
			return -1;
		}
		int SumFaces = 0;
		for (int i = 0; i < PM->GetNumSections(); i++)
		{
			SumFaces += PM->GetProcMeshSection(i)->ProcIndexBuffer.Num()/3;
			if (InHit.FaceIndex < SumFaces) return i;
		}
	}
	return -1;
}

FIntVector FTiledLevelUtility::GetImpactSizeCenter(EImpactSize ImpactSize)
{
	switch (ImpactSize) {
		...
		case EImpactSize::Five:
			return FIntVector(2, 2, 0);
		default:
			return FIntVector(0);
	}
}

int FTiledLevelUtility::GetImpactSizeExtent(EImpactSize ImpactSize)
{
	switch (ImpactSize) {
		case EImpactSize::One:
			return 1;
...
FVector FTiledLevelUtility::RotationXY(const FVector& InPos, float Degree)
{
	// in clockwise direction! -90 is what I would expect for clockwise...
	const float r = FMath::DegreesToRadians(Degree);
	return FVector(
		FMath::Cos(r) * InPos.X - FMath::Sin(r) * InPos.Y,
		FMath::Sin(r) * InPos.X + FMath::Cos(r) * InPos.Y ,
		InPos.Z);
}

FIntVector FTiledLevelUtility::RotationXY(const FIntVector& InPos, float Degree)
{
	// in clockwise direction! -90 is what I would expect for clockwise...
	const float r = FMath::DegreesToRadians(Degree);
	return FIntVector(
		FMath::RoundToInt(FMath::Cos(r) * InPos.X - FMath::Sin(r) * InPos.Y ),
		FMath::RoundToInt(FMath::Sin(r) * InPos.X + FMath::Cos(r) * InPos.Y ),
		InPos.Z);
}
