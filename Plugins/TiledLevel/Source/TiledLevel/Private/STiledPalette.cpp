                              ...
                              FTiledLevelEdMode* InEdMode, UTiledItemSet* InItemSet)
{
	...
	SetupFillDetailWidget();

	
	SAssignNew(PaletteDetailSwitcher, SWidgetSwitcher);
	// TODO: Force always show scroll bar finally fixed can not scroll issue...
	// Not the best solution... (it won't hide when not necessary)...
	// the implementation in foliage edit just won't work here....
	PaletteDetailSwitcher->AddSlot(ETiledPaletteDetailViewMode::ItemDetail)[
		SNew(SScrollBox)
		...
		[
			...
			[
				...
				.ToolTipText(LOCTEXT("Add_Special_Items_ToolTip",
				                     "Add special items for particular purpose!"))
				.ContentPadding(0)
				.ForegroundColor(FSlateColor::UseForeground())
				.ButtonStyle(FAppStyle::Get(), "ToggleButton")
	...
	/**
	 * Content differences comparison:
	 * Common: Header Row (O), Palette Tiles (0)
	 * Set Editor: Drop to add, Detail Widget 100%, Special item configurationss.
	 * Asset Editor: Detail Widget Transform only, Fill detail
	 */
	...
	[
		...
		+ SVerticalBox::Slot()
		.AutoHeight()
		.MaxHeight(TAttribute<float>::CreateLambda([this](){return bHideDetail? 0 : 360;}))
		[
			 SNew(SOverlay)
			 // actual Palettes
			 + SOverlay::Slot()
			 [
				  ...
				  [
					  ...
					  [
						   SNew(SButton)
						   .Visibility(this, &STiledPalette::GetVisibilityBasedOnEditorType, false)
		...
		[
			 ...
			 [
				  SNew(SBox)
				  .Visibility(this, &STiledPalette::GetVisibilityBasedOnEditorType, true)
				  [
...
EVisibility STiledPalette::GetVisibilityBasedOnEditorType(bool ForSetEditor) const
{
	if (ForSetEditor)
	{
		return SetEditor.IsValid() ? EVisibility::Visible : EVisibility::Collapsed;
	}
	return SetEditor.IsValid() ? EVisibility::Collapsed : EVisibility::Visible;
}
...
bool STiledPalette::PassesOptionFilter(UTiledLevelItem* InItem)
{
	...
	if (InItem->IsA<UTiledLevelRestrictionItem>() && !Settings->IsItemsVisible_Restriction) return false;
	if (InItem->IsA<UTiledLevelTemplateItem>() && !Settings->IsItemsVisible_Template) return false;
	...
	if (EdMode)
	{
		...
		{
			if (InItem->IsSpecialItem())
			{
				PassEraserFilter = false;
			}
			else
			{
				if (EdMode->IsFillTiles)
				{
					PassFillFilter = InItem->PlacedType == EPlacedType::Block || InItem->PlacedType ==
					                 EPlacedType::Floor;
				}
				else
				{
					PassFillFilter = InItem->PlacedType == EPlacedType::Edge || InItem->PlacedType == EPlacedType::Wall;
				}
			}
		}
	}
...
TSharedRef<SWidget> STiledPalette::GetViewOptionMenuContent()
{
	...
	MenuBuilder.BeginSection("TiledItemFilterOption", LOCTEXT("FilterOptionHeading", "Filter Option"));
	{
		...
		MenuBuilder.AddWidget(
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("SpecialItemFilterOptionLabel_Restriction", " Restrict"))
				.Font(FCoreStyle::GetDefaultFontStyle("Regular", 8))
			]
			+ SHorizontalBox::Slot()
			.VAlign(VAlign_Center)
			...
			[
				SNew(SCheckBox)
				.IsChecked_Raw(this, &STiledPalette::GetFilterOptionState_Restriction)
				.OnCheckStateChanged_Raw(this, &STiledPalette::SetFilterOptionState_Restriction)
			]
			+ SHorizontalBox::Slot()
			.VAlign(VAlign_Center)
			.HAlign(HAlign_Right)
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("SpecialItemFilterOptionLabel_Template", "Template"))
				.Font(FCoreStyle::GetDefaultFontStyle("Regular", 8))
			]
			+ SHorizontalBox::Slot()
			.VAlign(VAlign_Center)
			...
			[
				SNew(SCheckBox)
				.IsChecked_Raw(this, &STiledPalette::GetFilterOptionState_Template)
				.OnCheckStateChanged_Raw(this, &STiledPalette::SetFilterOptionState_Template)
			]
			,
			LOCTEXT("SpecialItemsFilterLabel", "    Special Items:")
		);
		...
		MenuBuilder.AddWidget(
			...
			[
				SNew(SButton)
				.OnClicked(this, &STiledPalette::OnResetFilterClicked)
				[
					SNew(STextBlock)
	...
	{
		// Can only use padding to resize widget here...
		MenuBuilder.AddWidget(
			SNew(SBox)
			.Padding(FMargin(32, 0))
			[
				SNew(SSlider)
...
FReply STiledPalette::OnShowHideDetailsClicked()
{
	const bool DetailWidgetCurrentlyVisible = PaletteDetailSwitcher->GetVisibility() != EVisibility::Collapsed;
	PaletteDetailSwitcher->SetVisibility(DetailWidgetCurrentlyVisible? EVisibility::Collapsed : EVisibility::Visible);
	bHideDetail = !bHideDetail;
	return FReply::Handled();
}

const FSlateBrush* STiledPalette::GetShowHideDetailsImage() const
{
	const bool bDetailsCurrentlyVisible = PaletteDetailSwitcher->GetVisibility() != EVisibility::Collapsed;
...
DEFINE_FILTER_FUNCTIONS(Restriction)
DEFINE_FILTER_FUNCTIONS(Template)

FReply STiledPalette::OnResetFilterClicked()
{
	UTiledLevelSettings* Settings = GetMutableDefault<UTiledLevelSettings>();
	Settings->IsItemsVisible_Block = true;
	Settings->IsItemsVisible_Floor = true;
	Settings->IsItemsVisible_Wall = true;
	Settings->IsItemsVisible_Edge = true;
	Settings->IsItemsVisible_Pillar = true;
	Settings->IsItemsVisible_Point = true;
	Settings->IsItemsVisible_Structure = true;
	Settings->IsItemsVisible_Prop = true;
	Settings->IsItemsVisible_Actor = true;
	Settings->IsItemsVisible_Mesh = true;
	Settings->IsItemsVisible_Restriction = true;
	Settings->IsItemsVisible_Template = true;
	UpdatePalette(false);
	return FReply::Handled();
}
...
{
	if (!SetEditor.IsValid()) return EVisibility::Collapsed;
...
void STiledPalette::SetupFillDetailWidget()
{
	...
	.HeaderRow
	(
		SNew(SHeaderRow) // No label should be good enough~
		+ SHeaderRow::Column(FillDetailTreeColumns::ColumnID_Thumbnail)
		.HeaderContentPadding(FMargin(0, 1, 0, 1))
		.DefaultLabel(FText::GetEmpty())
		.HAlignHeader(HAlign_Center)
		
		+ SHeaderRow::Column(FillDetailTreeColumns::ColumnID_RandomRotation)
		.DefaultLabel(LOCTEXT("FillRandomRotation_Label","Random Rotation?"))
		.DefaultTooltip(LOCTEXT("FillWithRotationTooltip","Place filled item with random yaw rotation?"))
		.HeaderContentPadding(FMargin(10, 1, 0, 1))
		.HAlignHeader(HAlign_Center)
		.HAlignCell(HAlign_Center)
		
		+ SHeaderRow::Column(FillDetailTreeColumns::ColumnID_AllowOverlay)
		.DefaultLabel(LOCTEXT("AllowOverlay_Label","Allow Overlay?"))
		.DefaultTooltip(LOCTEXT("AllowOverlayTooltip","Allow overaly with other tile? (Still only one structural item per unit)"))
		.HeaderContentPadding(FMargin(10, 1, 0, 1))
		.HAlignHeader(HAlign_Center)
		.HAlignCell(HAlign_Center)

		+ SHeaderRow::Column(FillDetailTreeColumns::ColumnID_Coefficient)
		.DefaultLabel(LOCTEXT("FillCoefficient_Label","Coefficient | Actual Ratio"))
		.DefaultTooltip(LOCTEXT("FillCofficientTooltip","Determine relative area filled with this item"))
		.HeaderContentPadding(FMargin(10, 1, 0, 1))
		.HAlignHeader(HAlign_Center)
		.HAlignCell(HAlign_Center)
...
EVisibility STiledPalette::GetCreateCustomDataRowButtonVisibility() const
{
	...
	{
		if (SelectedItems[0]->Item->IsSpecialItem())
