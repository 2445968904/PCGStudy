...
void FTiledItemDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	else
	{
		...
		// customize pivot options and auto fit
		...
		{
			DetailBuilder.HideProperty(PropertyHandle_bAutoPlacement);
		}
		...
		IDetailGroup& AutoSnap_Group = PlacementCategory.AddGroup(FName("AutoSnap"),
			FText::FromString("Auto Snap"),false, true);
	...
	// INSTANCE SETTINGS
	// Add this back when I can fix the Mobility bug one day...
	// IDetailCategoryBuilder& InstanceCategoryBuilder = DetailBuilder.EditCategory("InstanceSettings");
	// InstanceCategoryBuilder.AddCustomBuilder(MakeShared<FMobilityCustomization>(
	// 	DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UTiledLevelItem, Mobility)),
	// 	FMobilityCustomization::StationaryMobilityBitMask,
	// 	false),
	// 	false);
