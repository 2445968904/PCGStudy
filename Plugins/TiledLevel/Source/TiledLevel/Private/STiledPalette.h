...
private:
	...
	EVisibility GetVisibilityBasedOnEditorType(bool ForSetEditor = false) const;
	...
	FReply OnShowHideDetailsClicked();
	const FSlateBrush* GetShowHideDetailsImage() const;
	
	bool bHideDetail = false;
...
#define DECLARE_FILTER_FUNCTIONS(Target) \
	...
	DECLARE_FILTER_FUNCTIONS(Restriction)
	DECLARE_FILTER_FUNCTIONS(Template)

	FReply OnResetFilterClicked();
