...
FTiledItemSetEditor::FTiledItemSetEditor()
	: TiledItemSetBeingEdited(nullptr), ActiveItem(nullptr), PreviewSceneActor(nullptr),
                                                 ...
                                                 UTiledItemSet* InItemSet)
{
	...
	const TSharedRef<FTabManager::FLayout> StandaloneDefaultLayout = FTabManager::NewLayout(
			"Standalone_TiledElementSetEditor_Dev013")
		->AddArea
		(
...
void FTiledItemSetEditor::UpdatePreviewScene(UTiledLevelItem* TargetItem)
{
	if (TargetItem)
	{
		...
		if (TargetItem->SourceType == ETLSourceType::Mesh)
		{
			...
			TargetItem->UpdatePreviewMesh.BindSP(this, &FTiledItemSetEditor::UpdatePreviewMeshInEditor, TargetItem);
			UpdatePreviewMeshInEditor(TargetItem);
		}
		else
		{
		    ...
		    else if (UTiledLevelTemplateItem* TemplateItem = Cast<UTiledLevelTemplateItem>(ActiveItem))
            {
		        ...
		        if (UTiledLevelAsset* Asset = TemplateItem->GetAsset())
		        {
		            ...
		            Ash->PopulateSelected(true);
		            ActiveItem->Extent = FVector(Ash->GetSelectedExtent(true));
                    FTiledLevelUtility::ApplyPlacementSettings_TiledActor(TiledItemSetBeingEdited->GetTileSize(), ActiveItem,
                         PreviewBrush, PreviewCenter);
		        }    
            }
...
TSharedRef<SDockTab> FTiledItemSetEditor::SpawnTab_ContentBrowser(const FSpawnTabArgs& Args)
{
	...
	// Use fixed ID instead of a uid afterward fixed popping of unrecognizable tab..
	const FName ContentBrowserID = TEXT("TiledItemSetEditor_ContentBrowser");
	...
	const TSharedRef<SWidget> ContentBrowser = ContentBrowserSingleton.CreateContentBrowser(
...
void FTiledItemSetEditor::UpdatePreviewMeshInEditor(UTiledLevelItem* Item)
{
	...
	FTiledLevelUtility::ApplyPlacementSettings(TiledItemSetBeingEdited->GetTileSize(), Item, PreviewMesh,
		PreviewBrush, PreviewCenter);
