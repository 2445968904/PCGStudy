...
EVisibility FTiledItemViewData::GetEraserCheckBoxVisibility() const
{
	if (EdMode)
	{
		return EdMode->ActiveEditTool==ETiledLevelEditTool::Eraser && EdMode->GetPalettePtr()->AnyTileHovered()? EVisibility::Visible : EVisibility::Collapsed;
	}
...
FLinearColor STiledPaletteItem::GetTileColorAndOpacity() const
{
	if (EdMode)
	{
		if (Data->Item->bIsEraserAllowed && EdMode->ActiveEditTool == ETiledLevelEditTool::Eraser)
