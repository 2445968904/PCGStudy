...
void FTiledLevelEditorUtility::ConfigThumbnailAssetColor(FAssetThumbnailConfig& ThumbnailConfig, UTiledLevelItem* Item)
{
	if (!Item) return;
    if (Item->IsSpecialItem())
    {
        ThumbnailConfig.AssetTypeColorOverride = GetMutableDefault<UTiledLevelSettings>()->SpecialItemColor;
    }
	else if (Item->StructureType == ETLStructureType::Structure)
	{
		if (Item->SourceType == ETLSourceType::Actor)
			ThumbnailConfig.AssetTypeColorOverride = GetMutableDefault<UTiledLevelSettings>()->StructureActorItemColor;
		else
			ThumbnailConfig.AssetTypeColorOverride = GetMutableDefault<UTiledLevelSettings>()->StructureMeshItemColor;
	}
	else
	{
		if (Item->SourceType == ETLSourceType::Actor)
			ThumbnailConfig.AssetTypeColorOverride = GetMutableDefault<UTiledLevelSettings>()->PropActorItemColor;
		else
			ThumbnailConfig.AssetTypeColorOverride = GetMutableDefault<UTiledLevelSettings>()->PropMeshItemColor;
	}
}
