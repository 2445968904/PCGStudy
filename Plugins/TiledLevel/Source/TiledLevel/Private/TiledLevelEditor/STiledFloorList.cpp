...
void STiledFloorList::Construct(const FArguments& InArgs, UTiledLevelAsset* InTiledLevel, TSharedPtr<class FUICommandList> InParentCommandList)
{
	...
	FToolBarBuilder ToolbarBuilder(CommandList, FMultiBoxCustomization("TiledLevelFloorBrowserToolbar"),
	...
	ToolbarBuilder.AddToolBarButton(GenericCommands.Duplicate, NAME_None, TAttribute<FText>(),
		LOCTEXT("DuplicateFloorTooltip", "Duplicate selected floor."), DuplicateIcon);
	...
	ToolbarBuilder.AddToolBarButton(GenericCommands.Delete, NAME_None, TAttribute<FText>(),
		LOCTEXT("DeleteFloorTooltip", "Delete selected floor."), DeleteIcon);
...
TSharedRef<ITableRow> STiledFloorList::OnGenerateLayerListRow(FMirrorEntry Item, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	TSharedRef<RowType> NewRow = SNew(RowType, OwnerTable)
		.Style(&FTiledLevelStyle::Get().GetWidgetStyle<FTableRowStyle>("TiledLevel.FloorBrowser.TableViewRow"));
...
TSharedPtr<SWidget> STiledFloorList::OnConstructContextMenu()
{
	...
	MenuBuilder.BeginSection("InstanceOperations", LOCTEXT("InstaceOperationsHeader", "Instance actions"));
	{
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ClearAllInstace", "Clear all instance"),
		...
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ClearNormalPaint", "Clear normal paint instances"),
			LOCTEXT("ClearNormalPaintTooltip", "Clear normal paint instances in this floor"),
			DummyIcon,
			FUIAction(FExecuteAction::CreateRaw(this, &STiledFloorList::ClearNormalInstances))
			);
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ClearAutoPaint", "Clear auto paint instances"),
			LOCTEXT("ClearAutoPaintInstancesTooltip", "Clear auto paint instances in this floor"),
			DummyIcon,
			FUIAction(FExecuteAction::CreateRaw(this, &STiledFloorList::ClearAutoInstances))
			);
...
void STiledFloorList::MoveFloorUp(bool bForceToTop)
{
	if (UTiledLevelAsset* TiledLevelAsset =  TiledLevelAssetPtr.Get())
	{
		const FScopedTransaction Transaction(LOCTEXT("TiledLevelReorderFloorTransaction", "Reorder Floor"));
		TiledLevelAsset->SetFlags(RF_Transactional);
		TiledLevelAsset->Modify();
		const int32 SelectedIndex = GetSelectionPosition();
		const int32 MaxPosition = GetMaxFloorPosition();
		TiledLevelAsset->MoveFloor(true, SelectedIndex, MaxPosition, bForceToTop);
...
void STiledFloorList::MoveFloorDown(bool bForceToBottom)
{
	if (UTiledLevelAsset* TiledLevelAsset =  TiledLevelAssetPtr.Get())
	{
		...
		TiledLevelAsset->MoveFloor(false, SelectedIndex, MinPosition, bForceToBottom);
...
{
	const FScopedTransaction Transaction(NSLOCTEXT("UnrealEd", "TiledLevelMode_ClearInstanceTransaction", "Clear tiled item instance in floor"));
	...
	TiledLevelAssetPtr.Get()->ClearAutoPaintDataForFloor(GetActiveFloor()->FloorPosition);
	OnResetInstances.Execute();
}

void STiledFloorList::ClearNormalInstances() const
{
	const FScopedTransaction Transaction(NSLOCTEXT("UnrealEd", "TiledLevelMode_ClearNormalInstanceTransaction", "Clear normal tiled item instances in floor"));
	TiledLevelAssetPtr.Get()->Modify();
	GetActiveFloor()->BlockPlacements.Empty();
	GetActiveFloor()->FloorPlacements.Empty();
	GetActiveFloor()->WallPlacements.Empty();
...
void STiledFloorList::ClearAutoInstances() const
{
	const FScopedTransaction Transaction(NSLOCTEXT("UnrealEd", "TiledLevelMode_ClearAutoInstanceTransaction", "Clear auto tiled item instances in floor"));
	TiledLevelAssetPtr.Get()->Modify();
	TiledLevelAssetPtr.Get()->ClearAutoPaintDataForFloor(GetActiveFloor()->FloorPosition);
}
