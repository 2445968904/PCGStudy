...
void FTiledLevelDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	(
		...
		[
			...
			[
				...
				.Text_Lambda([this]
				{
					return IsInstanced() ? LOCTEXT("Instanced", "Instanced"): LOCTEXT("Asset", "Asset");
				})
	...
	[
		...
		[
			...
			.ToolTipText(LOCTEXT("PromoteToInstanceToolTip", "Convert to unique object that can be edited directly in level editor"))
		]
	]
	...
	[
		...
		[
			...
			.ToolTipText(LOCTEXT("EditAsInstaceTooltip","Edit this tiled level instance directly in current viewport."))
		]
	]
	...
	if (IsInstanced() || !bEditingActor)
	{
		...
		[
			SNew(SVerticalBox)
			.Visibility(this, &FTiledLevelDetailCustomization::GetVisibilityBasedOnIsInstanceEditing)
			+ SVerticalBox::Slot()
			.AutoHeight()
			[
				SNew(STextBlock)
			...
			+ SVerticalBox::Slot()
			[
				SAssignNew(FloorListWidget, STiledFloorList, TiledLevelAsset, CommandList)
	...
	if (bEditingActor)
	{
		...
		if (TiledLevelAsset && TiledLevelAsset->GetAutoPaintRule())
		{
			TiledLevelCategory
			...
			[
				...
				[
					 SNew(STextBlock)
					 .Text(LOCTEXT("ViewAsLabel", "View as: "))
				]
				+ SHorizontalBox::Slot()
				[
					 SNew(SCheckBox)
					 ...
					 .IsChecked_Lambda([=]()
					 {
						  return TiledLevelAsset->ViewAsAutoPaint? ECheckBoxState::Unchecked : ECheckBoxState::Checked;
					 })
					 .OnCheckStateChanged_Lambda([=](ECheckBoxState NewState)
					 {
					 	if (NewState == ECheckBoxState::Checked)
							TiledLevelAsset->SetNewViewState(false);
					 })
					 .ToolTipText(LOCTEXT("ViewAsNormalTooltip", "Display normal paint instances"))
					 [
					     SNew(STextBlock)
					     .Text(LOCTEXT("Normal", "Normal"))
					 ]
				]
				+ SHorizontalBox::Slot()
				[
					 ...
					 .IsChecked_Lambda([=]()
					 {
						 return TiledLevelAsset->ViewAsAutoPaint? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
					 })
					 .OnCheckStateChanged_Lambda([=](ECheckBoxState NewState)
					 {
					 	 if (NewState == ECheckBoxState::Checked)
							 TiledLevelAsset->SetNewViewState(true);
					 })
					 .ToolTipText(LOCTEXT("ViewAsAutoPaintTooltip", "Display auto paint results"))
					 [
						  SNew(STextBlock)
...
// TODO: this term is so confusing... what is instanced??
...
EVisibility FTiledLevelDetailCustomization::GetVisibilityBasedOnIsInstanceEditing() const
{
	if (TiledLevelActorPtr.IsValid())
	{
		return TiledLevelActorPtr->GetAsset()->CanEditTileNum? EVisibility::Visible : EVisibility::Collapsed;
	}
...
FReply FTiledLevelDetailCustomization::OnEditInLevelClicked()
{
	...
	EdMode->SetupData(TiledLevelActorPtr.Get()->GetAsset(), TiledLevelActorPtr.Get(), Helper, nullptr, false, false);
	MyDetailLayout->ForceRefreshDetails();
	return FReply::Handled();
}
