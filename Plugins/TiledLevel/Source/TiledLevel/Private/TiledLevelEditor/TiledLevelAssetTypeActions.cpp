	...
	EToolkitMode::Type Mode = EditWithinLevelEditor.IsValid() ? EToolkitMode::WorldCentric : EToolkitMode::Standalone;
 	for (UObject* obj : InObjects)
 	{
 		if (UTiledLevelAsset* TileLevel = Cast<UTiledLevelAsset>(obj))
 		{
			...
			NewTiledLevelEditor->InitTiledLevelEditor(Mode, EditWithinLevelEditor, TileLevel);
 		}		
 	}	
}
...
UThumbnailInfo* FTiledLevelAssetTypeActions::GetThumbnailInfo(UObject* Asset) const
{
    ...
    if (ThumbnailInfo == NULL)
    {
    	...
    	Cast<USceneThumbnailInfo>(ThumbnailInfo)->OrbitZoom = 150; // fix thumbnail not appear? when first create and not edit thumbnail view params ever??
