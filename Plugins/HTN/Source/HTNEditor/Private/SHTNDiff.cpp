...
#if UE_VERSION_OLDER_THAN(5, 1, 0)
	PanelOld.GeneratePanel(PanelNew.HTN->HTNGraph);
	PanelNew.GeneratePanel(PanelOld.HTN->HTNGraph);
#else
	PanelOld.GeneratePanel(FoundDiffs);
	PanelNew.GeneratePanel(FoundDiffs);
#endif
...
void SHTNDiff::OnSelectionChanged(FSharedDiffOnGraph Item, ESelectInfo::Type SelectionType)
{
	...
	else if (Result.Node1)
	{
		...
		const auto FocusNode = [this](UEdGraphNode* InNode)
		{
			...
			{
				UHTNGraphNode_Decorator* const DecoratorGraphNode = Cast<UHTNGraphNode_Decorator>(HTNGraphNode);
				UHTNGraphNode_Service* const ServiceGraphNode = Cast<UHTNGraphNode_Service>(HTNGraphNode);
...
void SHTNDiff::FHTNDiffPanel::GeneratePanel(
#if UE_VERSION_OLDER_THAN(5, 1, 0)
...
#endif
{
	...
	else
	{
		...
		const FSlateBrush* ContentAreaBrush = FHTNAppStyle::GetBrush("Docking.Tab", ".ContentAreaBrush");
		SAssignNew(Widget, SSplitter)
