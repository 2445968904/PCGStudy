...
void AASStaticMeshDistributionSpline::CheckForSurfaceSnapping()
{
	if(SnapToSurface)
	{
		if(!GWorld){return;}
		...
		if(InstanceType == ENonDeformInstanceType::Sm)
		{
			...
			for(const auto CurrentSMComp : FoundMesComps)
			{
				if(USceneComponent* LocalSmComp = Cast<USceneComponent>(CurrentSMComp))
				{
					...
					GWorld->LineTraceSingleByObjectType(HitResult, CompLoc, CompLoc + FVector(0,0,-15000.0f), objectParams, collisionParams);
			...
			for(const auto CurrentIsmComp : FoundInstSMComps)
			{
				if(UInstancedStaticMeshComponent* LocalIsmComp = Cast<UInstancedStaticMeshComponent>(CurrentIsmComp))
				{
					...
					for(int32 Index = 0; Index < Num ; ++Index)
					{
						...
						GWorld->LineTraceSingleByObjectType(HitResult, CompTransform.GetLocation(), CompTransform.GetLocation() + FVector(0,0,-15000.0f), objectParams, collisionParams);
						if(HitResult.bBlockingHit)
						{
...
AActor* AASStaticMeshDistributionSpline::SpawnAndGetAnActor(const FTransform& InTransform,const FString& InActorLabel, UWorld* InWorld) const
{
	...
	const auto SpawnedActor = InWorld->SpawnActor<AActor>(AActor::StaticClass(),InTransform,SpawnParams);
