...
void FODDistributionDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	IDetailCategoryBuilder& SimulationCategory = DetailBuilder.EditCategory(("Simulation"),FText::GetEmpty(),static_cast<ECategoryPriority::Type>(2));
	IDetailCategoryBuilder& SpawnCenterCategory = DetailBuilder.EditCategory(("SpawnCenter"),FText::GetEmpty(),static_cast<ECategoryPriority::Type>(3));
	IDetailCategoryBuilder& SettingsCategory = DetailBuilder.EditCategory(("Settings"),FText::GetEmpty(),static_cast<ECategoryPriority::Type>(4));
	...
	if(ToolSubsystem->bInMixerMode)
	{
		const TSharedRef<IPropertyHandle> ObjectDistributionData = DetailBuilder.GetProperty("ObjectDistributionData");
		DetailBuilder.HideProperty(ObjectDistributionData);
	}
	else
	{
		const TSharedRef<IPropertyHandle> bOverridePresetData = DetailBuilder.GetProperty("bOverridePresetProperties");
		DetailBuilder.HideProperty(bOverridePresetData);
		const TSharedRef<IPropertyHandle> CustomPresetData = DetailBuilder.GetProperty("CustomPresetData");
		DetailBuilder.HideProperty(CustomPresetData);
		const TSharedRef<IPropertyHandle> EditablePresetList = DetailBuilder.GetProperty("EditablePresetList");
		DetailBuilder.HideProperty(EditablePresetList);
	}

	const TSharedRef<IPropertyHandle> PropertiesHandle = DetailBuilder.GetProperty("bOverridePresetProperties");
	if (PropertiesHandle->IsValidHandle())
	{
		const FSimpleDelegate RefreshDelegate = FSimpleDelegate::CreateLambda([this, &DetailBuilder]()
		{
			DetailBuilder.ForceRefreshDetails();
		});
	...
	DetailBuilder.GetDetailsView()->SetIsPropertyVisibleDelegate(FIsPropertyVisible::CreateLambda([ODDistBaseObj](const FPropertyAndParent& InPropertyAndParent)
	{
		const auto ToolSubsystem = GEditor->GetEditorSubsystem<UODToolSubsystem>();
		if(!ToolSubsystem || !ODDistBaseObj){UE_LOG(LogTemp,Error,TEXT("Empty Pointer Data"));return true;}
		if(
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("EditablePresetList")))			||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("CustomPresetData")))			||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("MaxCollisionTest")))			||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("CollidingObjects")))			||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("TotalSpawnCount")))			||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("bOverrideDistributionData")))	||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("GlobalDistributionData")))		||
			InPropertyAndParent.Property.GetName().Equals(FString(TEXT("SpawnDensity"))))
		{
			if(ToolSubsystem->bInMixerMode)
			{
				if(ODDistBaseObj->bOverridePresetProperties)
				{
					return true;
				}
				return false;
...
#pragma region SpawnCenterButtons
	...
	[
		...
		.OnClicked_Lambda([ODDistBaseObj]() -> FReply
		{
			if(IsValid(ODDistBaseObj) && ODDistBaseObj->IsA(UODDistributionBase::StaticClass()))
			{
				ODDistBaseObj->SelectSpawnCenterPressed();
			}
			return FReply::Handled();
		})
	]
	...
	[
		...
		.OnClicked_Lambda([ODDistBaseObj]() -> FReply
		{
			if(IsValid(ODDistBaseObj) && ODDistBaseObj->IsA(UODDistributionBase::StaticClass()))
			{
				ODDistBaseObj->MoveSpawnCenterToWorldOriginPressed();
			}
			return FReply::Handled();
		})
	]
	...
	[
		...
		.OnClicked_Lambda([ODDistBaseObj]() -> FReply
		{
			if(IsValid(ODDistBaseObj) && ODDistBaseObj->IsA(UODDistributionBase::StaticClass()))
			{
				ODDistBaseObj->MoveSpawnCenterToCameraPressed();
			}
			return FReply::Handled();
		})
	]
